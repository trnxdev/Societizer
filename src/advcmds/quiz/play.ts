        ...
        .then(async (u: any) => {
            if (!u[0][0])
                return interaction.reply({
                    embeds: [
                        f.aembed('ошибка', 'Квиз не найден', f.colors.error),
                    ],
                    ephemeral: true,
                })
                    ...
                    async function QuizMe(
                        ...
                        if (i == quizData.length) {
                            ...
                            for (let i = 0; i < finBool.length; i++) {
                                ...
                                if (finBool[i] == true)
                                    embed.addField('Правильно', 'Да', true)
                                else
                                    embed.addField(
                                        'Правильный ответ',
                                        quizData[i].answers.filter(
                        ...
                        newmessage
                            ...
                            .on('collect', async (thismessage) => {
                                if (thismessage.user.id != interaction.user.id)
                                    return
                                if (thisAnsO[data.question] == null)
                                    thisAnsO[data.question] = (<any>(
                                        thismessage
                                    ))?.values[0]
