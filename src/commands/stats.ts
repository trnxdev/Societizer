...
export let command: Command = {
  ...
  run: async (interaction, client, f) => {
    ...
    cpuStat.usagePercent((_e: unknown, percent: number, _seconds: unknown) => {
      interaction.reply({ embeds: [new f.embed()
        .setTitle(`📦 | Статистика бота`)
        .addField(`Исп. ОЗУ`, `${Math.round(process.memoryUsage().heapUsed / 1024 / 1024)} МБ`, true)
        .addField(`Исп. Процессора`, `${percent.toFixed(2)}%`, true)
        .addField(`Время работы`, `${botuptime}`, true)
        .addField(`ОС`, "[Raspbian GNU/Linux 11](https://www.raspberrypi.com/software/)", true)
        .addField(`Хост`, "[Raspberry Pi 400](https://www.raspberrypi.com/products/raspberry-pi-400/)", true)
        .addField(`Запущен с помощью`, "[pm2](https://npmjs.com/pm2)", true)
        .addField(`ЯП`, `[Node.js ${process.version}](https://nodejs.org/)`, true)
        .addField(`Библиотека`, `[discord.js v${version}](https://discord.js.org/)`, true)
        .addField(`API Кластеризаций`, "[discord-hybrid-sharding](https://npmjs.com/package/discord-hybrid-sharding)", true)
