...
export let command: Command = {
  ...
  options: [
    {
      name: "поисковик",
      description: "поисковик с помощью которого вы хотите поискать запрос",
      type: 3,
      choices: [
        {
          name: "Google",
    ...
    if (ggl == "google") {
      search(query, { safe: true, additional_params: { hl: "ru" } }).then(
        (res: { results: [{title: string; url: string;}]}) => {
          ...
          return interaction.reply({ embeds: [resultsEmbed], ephemeral: anon });
        }
      );
      ...
      interaction.reply({ content: "Неизвестный поисковик", ephemeral: true });
  },
};
