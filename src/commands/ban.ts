...
export let command: Command = {
    ...
    run: async (interaction, client, f) => {
        ...
        const reason = interaction.options.getString("причина", false) || "Не указана";
        ...
        if (!member.permissions.has("BAN_MEMBERS"))
            return interaction.reply({
                embeds: [
                    f.aembed("Ошибка", "У вас нет прав на бан пользователей", f.colors.error),
                ],
                ephemeral: true,
            });
        if (!user)
        ...
        if (bans?.has(user.id))
            return interaction.reply({
                embeds: [
                    f.aembed(`Ошибка`, `Участник ${user.tag} уже забанен на данном сервере`, f.colors.error),
                ],
                ephemeral: true,
            });
        ...
        interaction.guild?.bans
            ...
            .then(() => {
            return interaction.reply({ embeds: [embed], ephemeral: anonban });
        }).catch((err) => {
