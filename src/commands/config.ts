...
export let command: Command = {
    ...
    run: async (interaction, client, f) => {
        db.promise().query(`SELECT * FROM guildconfig WHERE guildID='${interaction.guild!.id}'`).then(async (r: any) => {
            let data = r[0][0]
            ...
            selectionController.on("collect", async (i) => {
                if (i.customId === "suggestions.bot") {
                    let toggleSuggestions = new f.MessageButton().setLabel(`${(data.closedSuggestions == 1 ? "Включить" : "Выключить")} предложения`).setStyle("PRIMARY").setCustomId("suggestions.bot.toggle");
                    ...
                    let action = new f.MessageActionRow().addComponents([toggleSuggestions, selectChannel]);
                    ...
                    (<Message>await i.editReply({
                        ...
                        if (i.customId == "suggestions.bot.create") {
                            ...
                            db.promise().query(`UPDATE guildconfig SET closedSuggestions=${(data.closedSuggestions == 1 ? 0 : 1)} WHERE guildID='${interaction.guild!.id}'`).then(async () => {
                                await i.reply({
                                    embeds: [
                                        f.aembed("🔧 | Настройки сервера", `Предложения для сервера были успешно ${(data.closedSuggestions == 1 ? "включены" : "выключены")}.`, f.colors.default)
                                    ],
                                    ephemeral: true
                                })
                            })
