...
export let command: Command = {
    ...
    options: [
        ...
        },
        {
            name: 'анонимно',
            description:
                'Забанить пользователя анонимно, по стандарту не анонимно',
            type: 5,
        },
    ],
    run: async (interaction, client, f) => {
        ...
        let anonmute =
            interaction.options.getBoolean('анонимно', false) || false
        ...
        if (!muser)
            return interaction.reply({
                embeds: [
                    f.aembed(
                        'Ошибка',
                        `Пользователь которого вы хотите замьютить не найден на данном сервер`,
                        f.colors.error
                    ),
                ],
            })
        if (t && !f.parseTime(t))
            return interaction.reply({
                embeds: [
                    f.aembed(
                        'ошибка',
                        'Время неудалось конвертировать в мс!',
                        f.colors.error
                    ),
                ],
                ephemeral: true,
        ...
        if (
            ...
            return interaction.reply({
                embeds: [
                    f.aembed(
                        `Ошибка`,
                        `Роль участника которого вы указали выше чем роль бота`,
                        f.colors.error
                    ),
                ],
            })
        ...
        if (
            muser.roles.highest.position <= member.roles.highest.position &&
            ...
            return interaction.reply({
                embeds: [
                    f.aembed(
                        `Ошибка`,
                        `Ваша роль ниже чем роль участника которого вы указали`,
                        f.colors.error
                    ),
                ],
            })
