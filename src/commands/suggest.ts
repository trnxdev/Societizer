  ...
  run: (interaction, client, f) => {
    db.promise()
      ...
      .then(async (d: any) => {
        ...
        if (interaction.channel!.id != data.suggestionChannel)
          return interaction.reply({
            embeds: [
              f.aembed(
                "Ошибка",
                `Вы находитесь в неправильном канале (Зайдите в канал "<#${data.suggestionChannel}>")`,
                f.colors.error
              ),
            ],
            ephemeral: true,
        ...
        db.promise()
          .query(
            `INSERT IGNORE INTO suggestions(guildID, userSigned, author, date, messageID) VALUES ('${
