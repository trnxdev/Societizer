...
export let event: Event = {
  ...
  run: async (client: Client) => {
    client.guilds.cache.forEach((g) => {
      ...
      db.promise().query(`SELECT * FROM guildconfig WHERE guildID = '${g.id}'`).then((r: any) => {
        let data = r[0][0];
        if(data.suggestionTimeActive == 0) return;
        db.promise().query(`SELECT * FROM suggestions WHERE guildID='${g.id}' AND active='yes'`).then((u: any) => {
          u[0].forEach(async (s: any) => {
            let timePassed = new Date().getTime() - new Date(s.date).getTime();

            if (timePassed >= data.suggestionTimeActive) 
             doDef(client, data, s);
            else 
              setTimeout(() => doDef(client, data, s), data.suggestionTimeActive - timePassed);
          })
        })
      })
    });
...
export let doDef = async (client: any, data: any, s: any) => {
  ...
  let doneRow = new MessageActionRow().addComponents([doneButton, likedButton, dislikedButton, ShowStats]);

  (<TextChannel> await client.channels.cache.get(data.suggestionChannel))!.messages.fetch(s.messageID).then((m) => {
    m.edit({
      components: [doneRow]
    })
  })

  db.promise().query(`UPDATE suggestions SET active='no' WHERE suggestionID = '${s.suggestionID}'`);
}