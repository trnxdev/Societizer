    ...
    run: async (client: Client, interaction: Interaction) => {
        if (interaction.isCommand()) {
            ...
            try {
                ...
                console.log(
                    `[apps/${app.name}.ts, ${interaction.guild!.id}] ${
                        (<Error>e).message
                    }`
                )
            }
            ...
            interaction.component.type == 'BUTTON' &&
            interaction.component.customId?.startsWith('suggestion.')
        ) {
            let id =
            ...
            if (
                interaction.component.customId == 'suggestion.like' ||
                interaction.component.customId == 'suggestion.dislike'
            ) {
                let rating =
                    interaction.component.customId == 'suggestion.like' ? 1 : 0

                db.promise()
                    .query(
                        `SELECT * FROM suggestions WHERE suggestionID = '${id}'`
                    )
                    .then(async (res: any) => {
                ...
                db.promise()
                    ...
                    .then(async (res: any) => {
                        let signed = Object.values(
                            JSON.parse(res[0][0].userSigned)
                        )
